...
use crate::domain::primitives::{SourceText, LanguageId, DomainError, BytePos};
use crate::domain::events::{StructuralEvent, EventValidator};
use crate::domain::metrics::{FileMetrics, NodeMetrics, DuplicateGroup, DuplicateInstance};
...
impl SourceCode {
    ...
    pub fn add_events(&mut self, events: Vec<StructuralEvent>) -> Result<(), DomainError> {
        ...
        if !validator.is_balanced() {
            return Err(DomainError::InvalidSpan("unbalanced event stream".to_string()));
        }
    ...
    pub fn compute_metrics(&mut self) -> Result<&FileMetrics, DomainError> {
        if self.metrics.is_none() {
            ...
            let file_metrics = AggregationRules::compute_file_metrics(&self.events, &self.node_metrics, actual_loc)?;
    ...
    pub fn analyze_cursor(&self, cursor_pos: BytePos) -> Option<&NodeMetrics> {
        self.node_metrics.iter().find(|node| node.contains_position(cursor_pos))
    }
    
    pub fn validate_consistency(&self) -> Result<(), DomainError> {
        if let Some(metrics) = &self.metrics {
            let actual_loc = self.text.as_str().lines().count() as u32;
            let computed_metrics = AggregationRules::compute_file_metrics(&self.events, &self.node_metrics, actual_loc)?;
            
            if metrics != &computed_metrics {
                return Err(DomainError::InternalError("metrics inconsistency detected".to_string()));
            }
        }
    ...
    pub fn get_duplicate_groups(&self) -> Vec<DuplicateGroup> {
        ...
        for node in &self.node_metrics {
            ...
            fingerprint_map.entry(fp).or_default().push(DuplicateInstance {
        ...
        fingerprint_map
            ...
            .map(|(fingerprint, instances)| DuplicateGroup { fingerprint, instances })
            .collect()
    }
    ...
    fn extract_node_metrics(&self) -> Result<Vec<NodeMetrics>, DomainError> {
        ...
        let mut function_stack: Vec<(Option<String>, crate::domain::primitives::Span, usize)> = Vec::new();
            ...
            match event {
                ...
                StructuralEvent::FunctionEnd { .. } => {
                    if let Some((name, span, start_idx)) = function_stack.pop() {
                        ...
                        let fingerprint = DuplicationRules::compute_fingerprint_from_events(function_events);
...
mod tests {
    ...
    fn event_stream_validation() {
        ...
        let valid_events = vec![
            StructuralEvent::function_start(Some("test".to_string()), BytePos::new(0).unwrap(), BytePos::new(10).unwrap()).unwrap(),
            StructuralEvent::branch(BytePos::new(5).unwrap(), BytePos::new(8).unwrap(), BranchType::Conditional).unwrap(),
            StructuralEvent::function_end(BytePos::new(0).unwrap(), BytePos::new(10).unwrap()).unwrap(),
        ];
    ...
    fn metrics_computation() {
        ...
        let events = vec![
            StructuralEvent::function_start(Some("test".to_string()), BytePos::new(0).unwrap(), BytePos::new(20).unwrap()).unwrap(),
            StructuralEvent::branch(BytePos::new(12).unwrap(), BytePos::new(14).unwrap(), BranchType::Conditional).unwrap(),
            StructuralEvent::function_end(BytePos::new(0).unwrap(), BytePos::new(20).unwrap()).unwrap(),
        ];
